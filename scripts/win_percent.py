...
with open("data/runs_augmented.csv", "r") as r:
    ...
    for run in runs:
        ...
        if key not in win_rates:
            win_rates[key] = [int(run[12])] # 11 is won
        else:
            win_rates[key].append(int(run[12])) # 11 is won
...
with open("data/runs_augmented.csv", "r") as r:
    ...
    for line in lines:
        if line is lines[0]: # header row
            continue
        performances = win_rates[int(line[4])]  # 2 is horse_id, 3 is trainer_id, 4 is jockey_id
